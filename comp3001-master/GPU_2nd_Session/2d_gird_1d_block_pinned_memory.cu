...
int main(int argc, char* argv[])
{
	...
	cudaStatus = cudaMallocHost(&host_a, VECTOR_LENGTH * sizeof(int));
	...
	cudaStatus = cudaMallocHost(&host_b, VECTOR_LENGTH * sizeof(int));
	if (cudaStatus != cudaSuccess) {
		printf("\n memory not allocated");
		cudaFreeHost(host_a);
		return -1;
	...
	int* host_c;
	cudaStatus = cudaMallocHost(&host_c, VECTOR_LENGTH * sizeof(int));
	if (cudaStatus != cudaSuccess) {
		printf("\n memory not allocated");
