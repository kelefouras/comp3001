...
float  X1[M1], X2[M1], Y1[M1], test2[M1] __attribute__((aligned(64)));
...
unsigned short int Add_SSE() {
	...
	for (int i = 0; i < M1; i+=4) { //IMPORTANT: M MUST BE A MULTIPLE OF 4, OTHERWISE IT DOES NOT WORK
...
unsigned short int Add_AVX() {
	...
	__m256  ymm0, ymm1, ymm2, ymm3, ymm4;
		...
		for (int i = 0; i < M1; i += 8) { //IMPORTANT: M MUST BE A MULTIPLE OF 8, OTHERWISE IT DOES NOT WORK
