...
Compile with: gcc main.c convolution_layer_2D.c -o p -mavx2 -O3 -march=native -lm -D_GNU_SOURCE  -g  -pthread -fopenmp
*/
...
void load_bias_FP(){
    ...
    bias_array_FP = _mm_malloc( Output_depth_dim * sizeof(float),64);
...
int load_create_input_output_array_FP(){
    ...
    in_FP = _mm_malloc( input_size * sizeof(float),64);
    if (in_FP==NULL) {
...
#pragma omp parallel for private(in_subscript)
    ...
    out_FP = _mm_malloc( output_size * sizeof(float),64);
    if (out_FP==NULL) {
    ...
    out_to_compare_with_FP = _mm_malloc( output_size * sizeof(float),64);
...
int load_filter_array_FP(){
    ...
    filter_FP = _mm_malloc( filter_size * sizeof(float),64);
    if (filter_FP==NULL) {
